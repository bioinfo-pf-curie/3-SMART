...
#infile <- "/bioinfo/users/egirard1/PolyAseq/svn/project/script/pipeline/trunk/temp/refseq_mm10.txt" 
#out_dir <- "/bioinfo/users/egirard1/PolyAseq/svn/project/script/pipeline/trunk/temp/"

refseq <- read.table(infile,h=TRUE,comment.char="~",check.names=FALSE,stringsAsFactors=FALSE)
...
write.table(matrix(unlist(sapply(res_files,"[",1)),ncol=6,byrow=TRUE),paste(out_dir,"whole_gene.bed",sep="/"),quote=FALSE,row.names=FALSE,col.names=FALSE,sep="\t")
write.table(matrix(unlist(sapply(res_files,"[",2)),ncol=6,byrow=TRUE),paste(out_dir,"last_exon_gene.bed",sep="/"),quote=FALSE,row.names=FALSE,col.names=FALSE,sep="\t")
write.table(matrix(unlist(sapply(res_files,"[",3)),ncol=6,byrow=TRUE),paste(out_dir,"whole_gene_wo_last_exon.bed",sep="/"),quote=FALSE,row.names=FALSE,col.names=FALSE,sep="\t")
